...
import (
	...
	mona "github.com/monasuite/monautil"
...
func (w *MonacoinWallet) buildTx(amount int64, addr btc.Address, feeLevel wi.FeeLevel, optionalOutput *wire.TxOut) (*wire.MsgTx, error) {
	// Check for dust
	script, _ := maddr.PayToAddrScript(addr)
	...
	changeSource := func() ([]byte, error) {
		...
		script, err := maddr.PayToAddrScript(addr)
		if err != nil {
			return []byte{}, err
		}
		return script, nil
	...
	getKey := txscript.KeyClosure(func(addr btc.Address) (*btcec.PrivateKey, bool, error) {
		a, err := maddr.NewAddressPubKeyHash(addr.ScriptAddress(), w.params)
		if err != nil {
...
func (w *MonacoinWallet) buildSpendAllTx(addr btc.Address, feeLevel wi.FeeLevel) (*wire.MsgTx, error) {
	tx := wire.NewMsgTx(1)
...
func (w *MonacoinWallet) sweepAddress(ins []wi.TransactionInput, address *btc.Address, key *hd.ExtendedKey, redeemScript *[]byte, feeLevel wi.FeeLevel) (*chainhash.Hash, error) {
	...
	script, err := maddr.PayToAddrScript(internalAddr)
	if err != nil {
		return nil, err
	}
	...
	for _, in := range ins {
		...
		script, err := maddr.PayToAddrScript(in.LinkedAddress)
		if err != nil {
			return nil, err
		}
...
func (w *MonacoinWallet) createMultisigSignature(ins []wi.TransactionInput, outs []wi.TransactionOutput, key *hd.ExtendedKey, redeemScript []byte, feePerByte uint64) ([]wi.Signature, error) {
	...
	for _, out := range outs {
		scriptPubkey, err := maddr.PayToAddrScript(out.Address)
		if err != nil {
			return sigs, err
		}
...
func (w *MonacoinWallet) generateMultisigScript(keys []hd.ExtendedKey, threshold int, timeout time.Duration, timeoutKey *hd.ExtendedKey) (addr btc.Address, redeemScript []byte, err error) {
	...
	addr, err = maddr.NewAddressWitnessScriptHash(witnessProgram[:], w.params)
	if err != nil {
		return nil, nil, err
	}
...
func (w *MonacoinWallet) estimateSpendFee(amount int64, feeLevel wi.FeeLevel) (uint64, error) {
	...
	addr, err := maddr.DecodeAddress("ltc1q65n2p3r4pwz4qppflml65en4xpdp6srjwultrun6hnddpzct5unsyyq4sf", &chaincfg.MainNetParams)
	if err != nil {
		return 0, err
	}
